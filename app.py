...
default_file_path = "/workspaces/market-basket-analysis/retail_transactions_sample.csv"
...
def load_data():
    ...
    return preprocess_data(df_spark)
...
if "freq_df" in st.session_state and "rules_df" in st.session_state:
    ...
    with tabs[0]:
        ...
        df_top = freq_df.limit(top_n_items).toPandas()
    ...
    with tabs[1]:
        ...
        df_rules_display = rules_df.limit(top_n_rules).toPandas()
    ...
    with tabs[4]:
        ...
        if insights.empty:
            ...
            insights = df_rules_pd.sort_values(["lift", "confidence"], ascending=False).head(3)
        for _, row in insights.iterrows():
            st.markdown(f"""
                ...
                <strong>Recommendation:</strong> Consider bundling or promotions.
                </div>
